# Plan 2/7: deepseek_bridge.py (Security-Fixed + Smart Edition)

**Path:** `C:\Users\ab117\OneDrive\文件\OPENCODE\MCP\deepseek_bridge.py`

## Bug Fixes Applied
- ✅ Path traversal: strict `Path.resolve().relative_to()` check
- ✅ `.env` files no longer filtered out by `read_project_files`
- ✅ DuckDuckGo parser more robust (multiple fallback selectors)
- ✅ `sources` now returns real URLs with titles
- ✅ Lazy imports at top level (no more import inside functions)
- ✅ Supabase uses `anon key` for `supabase_select` (read-only safe)

## Security Fixes
- ✅ Path traversal: absolute path + strict prefix check
- ✅ No credentials leaked in error messages
- ✅ Rate limiting built into MCP

## New Smart Features
- `auto_detect_intent()` — analyzes request, returns tool list
- `smart_search()` — context-aware query optimization
- `batch_read_files()` — read multiple files by patterns
- `project_analyzer()` — full project structure + optimization report
- `code_beautifier()` — auto-format code
...
async def web_search(query: str, max_results: int = 5) -> str:
    """Search the web using DuckDuckGo. Returns results with source URLs."""
    ...
    # Try multiple CSS selectors for resilience
...
async def read_project_files(extension: str = "") -> str:
    """Recursively list all project files (optionally filter by extension e.g. '.py')."""
    results = []
...
async def read_file(file_path: str) -> str:
    """Read a text file (path relative to project root). Security: path traversal protected."""
...
async def project_analyzer() -> str:
    ...
    checks = [
        (".env", "⚠️ No .env file found (create from .env.example)"),
    ...
    for fname, msg in checks:
        lines.append(f"- {'✅' if (PROJECT_ROOT / fname).exists() else '❌'} {msg}")
...
async def code_beautifier(file_path: str) -> str:
    ...
    suggestions.append(f"\n📊 Stats: {total_lines} lines, {sum(len(l) for l in lines)} chars")
...
async def supabase_query(sql: str) -> str:
    """Execute SQL via Supabase. Requires SUPABASE_URL + SUPABASE_KEY env vars."""
